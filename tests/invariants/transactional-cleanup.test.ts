/**
 * Transactional factory cleanup tests.
 *
 ...
 *   container, the runtime's GPU device released, the opened dataset disposed
 *   exactly once — and rethrows the ORIGINAL error (identity + cause intact;
 *   cleanup never masks it).
 * - `createViewerRuntime(config)` destroys the never-returned runtime on any
...
import { createViewerRuntime } from "../../src/index";
import { datasetRegistry } from "../../src/registry";
import type { State } from "../../src/state/schema";

// ============================================================================
// SPY DATASET KIND — per-instance dispose spies, one URL-backed mesh resource
// ============================================================================
...
class SpyDataset extends Dataset {
  ...
  override async load(): Promise<void> {
    ...
    // A URL-backed mesh resource with no pre-parsed geometry: the generated
    // surface layer fetches `mem://surf-volume.obj` at scene-build time, so
    // the fetch stub below controls the layer's settle.
...
// ============================================================================
// createViewerRuntime — failed GPU init / mount destroys the runtime
// ============================================================================

describe("createViewerRuntime transactional cleanup", () => {
  test("missing WebGPU rejects and leaves the registry usable", async () => {
    ...
    stubWebGPU();
    const runtime = await createViewerRuntime({
      state : makeState(),
      views : { main: { type: "volume", canvas: makeFakeCanvas(), layers: [] } },
    });
