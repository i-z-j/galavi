/**
 * Dtype packing tests (src/utils/render/pack.ts, via the root entry):
 * normalization scale/offset per dtype and float16 encoding of raw samples
 * (uint8 LUT equivalence, signed offsets, zero padding). The OME-Zarr
 * adapter's tile/plane packing cases live in tests/unit/adapters/ome-zarr/.
 */
