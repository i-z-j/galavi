/**
 * Layer readiness notification tests.
 *
 ...
 * and on abort. Headless: no GPU, rAF stubbed as in the integration/viewer
 * harness.
 */
...
describe("view(id).whenLayerReady", () => {
  ...
  async function setup() {
    ...
    const layer = runtime.view("main").getLayer("a") as FakeAsyncLayer;
    return { runtime, layer };
  }

  test("pends while not ready, resolves when the layer signals ready", async () => {
    const { runtime: g, layer } = await setup();
    let settled = false;
  ...
  test("loadStatus tracks readiness for plain async layers", async () => {
    const { runtime: g, layer } = await setup();
