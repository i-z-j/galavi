...
import { createViewerRuntime, type ViewConfig, type ViewerRuntime } from "../../../src/viewer";
import { registerOverlay } from "../../../src/registry";
import { BaseOverlay } from "../../../src/primitives/overlay";
...
describe("setOverlayOptions runtime pass-through", () => {
  ...
  test("custom overlay types receive their options verbatim", async () => {
    ...
    const overlay = runtime.view("main").base.getOverlays()[0] as RecorderOverlay;
    expect(overlay.received).toEqual({ initial: true });

    runtime.view("main").setOverlayOptions("test-recorder", { custom: 42 });
