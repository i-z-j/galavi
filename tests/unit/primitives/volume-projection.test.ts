...
import { createViewerRuntime, type ViewerRuntime } from "../../../src/index";
import {
  VolumeLayer,
  VolumeLayerParams,
  optVolumeMode,
} from "../../../src/primitives/layer";
import type { LayerConfig } from "../../../src/state/schema";
...
describe("setRender({ volumeProjection }) through a ViewerRuntime instance", () => {
  ...
  test("propagates via the render-config path", async () => {
    ...
    runtime.layer("v")!.setRender({ volumeProjection: "minip" });
    expect(runtime.layer("v")!.config.render?.volumeProjection).toBe("minip");

    // The state entry feeds the layer through the standard applyConfig path.
    const desc = runtime.getState().layers!.find((l) => l.id === "v")!;
    const layer = runtime.view("main").getLayer("v") as VolumeLayer;
    layer.applyConfig(desc);
    expect(paramsMode(layer)).toBe(1);

    runtime.layer("v")!.setRender({ volumeProjection: "mean" });
    layer.applyConfig(runtime.getState().layers!.find((l) => l.id === "v")!);
    expect(paramsMode(layer)).toBe(2);
