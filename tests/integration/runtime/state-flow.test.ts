/**
 * ViewerRuntime commit/notify flow tests (headless).
 *
 * `createViewerRuntime` defers GPU init when no view has a canvas, so the full
...
import { createViewerRuntime, type ViewerRuntime } from "../../../src/viewer";
import type { ImagePyramid, State } from "../../../src/state/schema";
...
describe("ViewerRuntime commit/notify flow (headless)", () => {
  ...
  test("createViewerRuntime works without canvases and without WebGPU", async () => {
    runtime = await createViewerRuntime({
      state : initialState,
      views : { main: { type: "volume", layers: [] } },
    });
    expect(runtime).toBeDefined();
    expect(runtime.getViewConfig("main")?.type).toBe("volume");
  ...
  test("getState round-trips and returns defensive clones", async () => {
    runtime = await createViewerRuntime({ state: initialState, views: {} });

    const a = runtime.getState();
    const b = runtime.getState();
    expect(a).toEqual(b);
    expect(a).not.toBe(b);
    expect(a.exploration.camera.position).not.toBe(b.exploration.camera.position);

    a.exploration.camera.target[0] = 99;
    expect(runtime.getState().exploration.camera.target[0]).toBe(0.5);
  });
  ...
  test("subscribe fires on a state change with the committed state", async () => {
    ...
    runtime.setTarget([0.25, 0.5, 0.5]);

    expect(onState).toHaveBeenCalledTimes(1);
    expect(onState.mock.calls[0]![0].exploration.camera.target).toEqual([0.25, 0.5, 0.5]);
    expect(runtime.getState().exploration.camera.target).toEqual([0.25, 0.5, 0.5]);
  ...
  test("subscribe does not fire on a no-op nav-mode switch", async () => {
    ...
    runtime.setNavMode("orbit"); // already orbit — early return, no commit
    expect(onState).not.toHaveBeenCalled();
    expect(rafQueue.size).toBe(0);

    runtime.setNavMode("fly");
    expect(onState).toHaveBeenCalledTimes(1);
    expect(runtime.getState().exploration.camera.navMode).toBe("fly");
  });

  test("setTarget recomputes position in orbit mode and unsubscribe stops notify", async () => {
    runtime = await createViewerRuntime({ state: initialState, views: {} });
    const onState = vi.fn();
    const unsubscribe = runtime.subscribe(onState);

    const before = runtime.getState().exploration.camera.position;
    runtime.setTarget([0.1, 0.2, 0.3]);
    const after = runtime.getState().exploration.camera.position;
    expect(after).not.toEqual(before); // orbit mode: position recomputed around target
    expect(onState).toHaveBeenCalledTimes(1);

    unsubscribe();
    runtime.setTarget([0.5, 0.5, 0.5]);
    expect(onState).toHaveBeenCalledTimes(1);
  });
});

// ============================================================================
// updateLayers — ONE ATOMIC MULTI-LAYER TRANSACTION
// ============================================================================
...
describe("ViewerRuntime.updateLayers", () => {
  let rafQueue: Map<number, FrameRequestCallback>;
  let rafId: number;
  let runtime: ViewerRuntime | undefined;
  ...
  test("one batch is one commit: one subscriber notification, one scheduled render", async () => {
    runtime = await createViewerRuntime({ state: layerState, views: {} });
    const onState = vi.fn();
    runtime.subscribe(onState);
    ...
    const [first, second] = runtime.getState().layers!;
  ...
  test("data merges shallowly across the batch", async () => {
    runtime = await createViewerRuntime({ state: layerState, views: {} });
    runtime.updateLayers([{ id: "volume-c0", data: { url: "mem://other" } }]);
    const layer = runtime.getState().layers![0];
  ...
  test("single setters behave as before through the same patch logic", async () => {
    ...
    runtime.layer("volume-c0")!.setOptions({ selection: { c: 1 } });
    expect(onState).toHaveBeenCalledTimes(1);
    expect(rafQueue.size).toBe(1);
    expect(runtime.getState().layers![0].options?.selection).toEqual({ c: 1, t: 0 });

    runtime.layer("volume-c0")!.setRender({ visible: false });
    expect(onState).toHaveBeenCalledTimes(2);
    expect(runtime.getState().layers![0].render?.visible).toBe(false);
    expect(runtime.getState().layers![0].render?.color).toBe("#00B0FF");

    runtime.layer("volume-c0")!.setData({ url: "mem://x" });
    expect(onState).toHaveBeenCalledTimes(3);
    expect(runtime.getState().layers![0].data?.url).toBe("mem://x");
    expect(runtime.getState().layers![0].data?.pyramid).toBe(PYRAMID);

    // Array-valued option keys replace, never merge.
    runtime.layer("volume-c0")!.setOptions({ axes: ["x", "z"] });
    expect(runtime.getState().layers![0].options?.axes).toEqual(["x", "z"]);
  });
});
