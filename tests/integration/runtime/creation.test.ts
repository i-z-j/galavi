/**
 * createViewerRuntime creation-options tests: `{ signal, waitUntil }`.
 *
 ...
 *   DOMException by default) and the partially created runtime is destroyed
...
import { BaseLayer, type Geometry, type LayerParams } from "../../../src/primitives/layer";
...
/** Per-layer behavior, keyed by layer ID; set before runtime creation. */
...
function track(runtime: ViewerRuntime): ViewerRuntime {
...
describe('createViewerRuntime waitUntil: "layers"', () => {
  test('default ("mounted") resolves without waiting for layer readiness', async () => {
    const runtime = track(await createViewerRuntime({
      state : makeState([{ id: "a", type: "gated" }]),
      views : makeViews({ main: ["a"] }),
    }));
    expect(runtime.view("main").getLayerStatus("a")).toEqual({ status: "loading" });
  });
  ...
  test("one layer: pends until the layer reports structural readiness", async () => {
    let settled = false;
    const pending = createViewerRuntime(
      { state: makeState([{ id: "a", type: "gated" }]), views: makeViews({ main: ["a"] }) },
      { waitUntil: "layers" },
    ).then((runtime) => { settled = true; return track(runtime); });
    ...
    expect(runtime.view("main").getLayerStatus("a")).toEqual({ status: "ready" });
  });
  ...
  test("a recorded layer load failure rejects with the error and destroys the runtime", async () => {
    const marker = new Error("source exploded");
    behaviors.set("bad", { failWith: marker });

    await expect(createViewerRuntime(
...
describe("createViewerRuntime signal", () => {
  test("a pre-aborted signal rejects before GPU init — nothing allocated", async () => {
    ...
    expect(instances.size).toBe(0); // the runtime was never constructed
  });
  ...
  test("abort after resolution has no effect — the runtime is the caller's", async () => {
    const controller = new AbortController();
    const runtime = track(await createViewerRuntime(
      { state: makeState([]), views: makeViews({ main: [] }) },
      { signal: controller.signal, waitUntil: "layers" },
    ...
    await flushMicrotasks();
    expect(deviceDestroy).not.toHaveBeenCalled();
    expect(runtime.getViewConfig("main")).toBeDefined();
  });
});
