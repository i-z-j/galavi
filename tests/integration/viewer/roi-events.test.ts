...
/**
 * High-level Viewer ROI event tests: `viewer.on("roiChange" |
...
async function makeViewer(config: ViewerConfig): Promise<{ viewer: Viewer; container: HTMLDivElement }> {
  ...
  const viewer = await createViewer(canvas, { dataset: DESC, composition: { type: "slice" }, ...config });
  viewers.push(viewer);
...
describe("viewer ROI events", () => {
  test("one change fires one event with view/composition identity", async () => {
    ...
    expect(actives).toEqual([{ activeIndex: 1, viewId: "main", composition: "slice" }]);
    ...
    expect(actives).toEqual([
      { activeIndex: 1, viewId: "main", composition: "slice" },
      { activeIndex: null, viewId: "main", composition: "slice" },
    ]);
  });

  test("quad composition reports the interacting view id", async () => {
    const container = document.createElement("div");
    document.body.appendChild(container);
    const viewer = await createViewer(container, {
      dataset : DESC,
      composition : { type: "quad" },
      tools   : { roi: { rois: [BOX_A, BOX_B], activeIndex: 0 } },
    });
    viewers.push(viewer);
  ...
  test("subscriptions survive composition rebuilds", async () => {
    const { viewer, container } = await makeViewer({
      tools: { roi: { rois: [BOX_A, BOX_B], activeIndex: 0 } },
    });
    const changes: ViewerRoiChangeEvent[] = [];
