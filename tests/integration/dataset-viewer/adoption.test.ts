/**
 * Viewer Dataset-adoption ownership tests.
 *
...
import { OMEZarrDataset, openOMEZarrDataset } from "../../../src/dataset/adapters/ome-zarr";
...
/** A URL-backed mesh with no pre-parsed geometry: the generated surface layer fetches at scene-build time and fails (404), failing the rebuild. */
const CONFIG_FAIL: DatasetConfig = { type: KIND, source: "mem://spy-fail" };
...
class SpyDataset extends Dataset {
  ...
  override async load(): Promise<void> {
    ...
    if (this.config.source === CONFIG_FAIL.source) {
      this.resources = [{ id: "mesh", kind: "mesh", source: "mem://spy-fail.obj" }];
...
describe("Viewer dataset adoption", () => {
  ...
  test("successful adoption: the viewer owns the exact instance until destroy", async () => {
    ...
    expect(viewer.runtime!.getState().layers!.map((l) => l.id)).toEqual(["volume-c0"]);
    // The unified State stays pure JSON: the adopted dataset's declarative config.
    expect(viewer.getState().dataset).toEqual(CONFIG);
    expect(dataset.disposeSpy).not.toHaveBeenCalled();
  ...
  test("failed rebuild after adoption: the viewer disposes the adopted dataset", async () => {
    const viewer = track(await createViewer(makeFakeCanvas()));
    const dataset = new SpyDataset(CONFIG_FAIL);
    await dataset.load();
    // The generated surface layer's source fetch fails (no pre-parsed
    // geometry on the resource — the layer must fetch the URL itself).
    vi.stubGlobal("fetch", vi.fn(async () => ({ ok: false, status: 404, text: async () => "" })));

    // The promise rejects — and the CALLER still must not dispose: the
    // viewer did, when the rebuild failed.
    await expect(viewer.open(dataset)).rejects.toThrow("Surface fetch failed: 404");
...
describe("metadata-open counting", () => {
