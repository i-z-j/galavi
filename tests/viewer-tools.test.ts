/**
 * High-level tool config purity tests (API-4).
 *
 ...
 *   dropped by the `viewer.config` JSON mirror.
 * - Every high-level tool's options round-trip through JSON unchanged.
 *
 * Harness mirrors tests/viewer.test.ts: a stub Dataset kind + fake canvas +
 * fake WebGPU device (rAF stubbed — no render pass runs).
 */
...
// @ts-expect-error — callbacks are not part of the high-level tool config (API-4)
...
// @ts-expect-error — mode-override tool bags are equally JSON-only (API-4)
const badOverride: ViewerConfig = { modeOverrides: { slice: { tools: { roi: { onRoisChange: () => {} } } } } };

function configureProbes(viewer: Viewer): void {
  viewer.tool("roi").configure({ visible: true, enabled: true }); // serializable — OK
  // @ts-expect-error — configure rejects callback keys at compile time (API-4)
...
describe("high-level tool config rejects functions (API-4)", () => {
  ...
  test("mode overrides reject functions in tool bags", async () => {
    await expect(makeViewer({
      dataset: DESC_3D,
      modeOverrides: { slice: { tools: { roi: { onRoisChange: () => {} } as never } } },
    })).rejects.toThrow(/modeOverrides\.slice: tools\.roi\.onRoisChange is a function/);
  });
  ...
  test("viewer.view().configure rejects functions in tool bags", async () => {
    const viewer = await makeViewer({ dataset: DESC_3D });
    expect(() => viewer.view("slice").configure({
      tools: { roi: { onActiveIndexChange: () => {} } as never },
    })).toThrow(/modeOverrides\.slice: tools\.roi\.onActiveIndexChange is a function/);
  });
});
...
describe("high-level tool config stays pure JSON (API-4)", () => {
  test("every high-level tool's options round-trip through JSON and viewer.config", async () => {
    ...
    expect(emitted.tools).toEqual(tools);
  });
