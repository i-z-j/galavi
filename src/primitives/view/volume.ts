...
export class VolumeView extends BaseView {
  ...
  override forward(action: { type: string; payload?: unknown }): void {
    if (action.type === "key:down") {
      ...
      if (code === "KeyF" && this.runtime) {
        const mode = this.runtime.getState().exploration.camera.navMode;
        this.runtime.setNavMode(mode === "fly" ? "orbit" : "fly");
        return;
      }
    }
  ...
  private getTransitionCamera(
    ...
    if (rawT < 1) {
      requestAnimationFrame(() => this.runtime?.requestRender());
    } else {
