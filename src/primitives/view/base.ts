...
import type { BaseControl } from "../control/base";
...
/**
 * ViewOwner — everything a view touches on its owning runtime: the resolved
 * overlay theme, the state authority (`getState`/`setState` for controls,
 * `setNavMode` for the volume view's fly/orbit toggle), the render-request
 * channel, and the auto-rotate stop. `ViewerRuntime`
 * (src/viewer/runtime.ts) structurally satisfies this interface; primitives
 * depend on the narrow contract, never the concrete runtime class.
 */
...
export abstract class BaseView {
  ...
  private readonly overlayBinding = {
    ...
    getTheme: () => this.runtime?.theme ?? DEFAULT_THEME,
    getOwner: () => this.runtime,
   ...
   *   channel (async tile/geometry work, or the runtime's tracked-load
   *   fan-out) while `isReady` holds. If the layer's source
   ...
   *   (e.g. unknown source type, unsupported metadata, network/CORS
   ...
   *
   * Called by the owning ViewerRuntime's fan-out — the layer's
   * render-request channel is attached once at the runtime and each signal
  ...
  /**
   * Runtime fan-out entry: the shared runtime layer's tracked async
  ...
  /**
   * Layer instances are owned by the ViewerRuntime: one runtime
   * instance per state-layer ID, created at runtime construction and shared
   * by every referencing view. View membership management below never wires
   * or unwires the layer's render-request channel — the runtime attaches it
  ...
  /** Bind this view to its owner (the runtime — used for `requestRender`). */
  setOwner(runtime: ViewOwner): void {
  ...
  forward(action: Action): void {
    if (!this.runtime || this.localControls.length === 0) return;
