...
import type { LayerConfig, PhysicalSpace, State, Vec3 } from "../../../state/schema";
import {
  cameraAngles,
  cameraDistance,
  computePosition,
...
import type { ViewerRuntime, ViewerRuntimeConfig } from "../../../viewer/runtime";
import { BaseOverlay } from "../base";
import { SVG_NS, clamp, createFullscreenSvg, physicalBounds } from "../utils";

/**
 * The nested-loupe runtime factory, injected by `viewer/runtime` (the module
 * that owns the overlay bootstrap): a static value import here would close a
 * module cycle (overlay barrel → magnifier → viewer/runtime → overlay
 * barrel).
 */
type NestedRuntimeFactory = (config: ViewerRuntimeConfig) => Promise<ViewerRuntime>;
let nestedRuntimeFactory: NestedRuntimeFactory | undefined;

/** Bind the factory used to create the nested loupe runtime. Called by `viewer/runtime`. */
export function bindNestedViewerRuntime(factory: NestedRuntimeFactory): void {
  nestedRuntimeFactory = factory;
}
...
export class MagnifierOverlay extends BaseOverlay {
  // No static `overlayType`: one implementation serves two registry IDs
  // (`magnifier-2d` / `magnifier-3d`), so `ensureBuiltInOverlays` registers
  // both variants via explicit factories parameterized by dimension.
  ...
  private async mountNested(canvas: HTMLCanvasElement): Promise<void> {
    ...
    try {
      ...
      let instance: ViewerRuntime;
      try {
        if (!nestedRuntimeFactory) {
          throw new Error("viewer/runtime not loaded — the nested loupe runtime factory is unbound");
        }
        instance = await nestedRuntimeFactory({
          state: nestedState,
          theme: owner.theme,
          views: {
            magnifier: {
