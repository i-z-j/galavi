...
export interface TiledImageOptions {
   ...
   * wins and the volume layer's automatic preview-budget policy is
...
// === Dev-time unknown-option-key diagnostics ===
...
export abstract class TiledImageLayer extends BaseLayer {
   ...
   * for pathological z-chunk=1 pyramids). Identity by default. Must be
  ...
  /**
   * Load state: `"idle"` when no source is set, `"ready"` otherwise.
   * Sources are explicit configs — a `Data` without `pyramid`/`fetch` no
   * longer triggers any async resolution. A failed tracked load
