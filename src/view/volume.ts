...
import { DEPTH_FORMAT, DEFAULT_FOV, NEAR_CLIP_FACTOR, FAR_CLIP_FACTOR, MODE_TRANSITION_MS } from "../defaults";
...
export class VolumeView extends BaseView {
  ...
  protected renderFrame(state: State): void {
    ...
    // Update tiles using each layer's local space.
    const dist            = cameraDistance(cam);
    const effectiveScale  = maxExtent / dist;
    this.pipeline.updateTiles(
      this.layerEntries,
      effectiveScale,
      (layer) => this.transformPoint(layer.invModelMatrix, cam.target),
