 ...
 * galavi's format-neutral `Dataset` contract by {@link OMEZarrDataset}:
 *
 ...
 *                        first channel only
 *   - resource         — one primary `"image-pyramid"` resource carrying the
 *                        runtime pyramid/fetch pair plus the normalized
 *                        metadata compositions consume
 */
...
import type { AxisIndex, AxisMap } from "../../utils/math/axes";
import { makeFloat16Encoder } from "../../utils/render/pack";
import { registerDatasetAdapter } from "../../registry";
import {
  Dataset,
  buildContrastLimits,
  getChannelColor,
 ...
 * typed `registerDatasetAdapter`) with this format's exact config. The
 * augmentation targets `../../state/schema` — the single `DatasetConfigMap`
 * declaration the public `"galavi"` entry re-exports (transitively, via
 * `dataset/contract`) — so it survives declaration emission verbatim
 * (`dist/dataset/adapters/ome-zarr.d.ts` keeps the same relative specifier)
 * and merges with the exact interface every published `DatasetConfig`
 * reference resolves to. (External format packages still augment the public
 * `declare module "galavi"` name, which resolves to the same interface
 * through the root re-export.)
 */
declare module "../../state/schema" {
...
/**
 * OMEZarrDataset — the `"ome-zarr"` dataset kind: a single OME-Zarr
 * multiscale image opened from a URL. `load()` normalizes the OME/OMERO
 * metadata onto the base Dataset fields and exposes one primary
 * `"image-pyramid"` resource carrying the runtime `pyramid`/`fetch` pair
 * (which also plug directly into layer `data` configs). Self-registers on
 * module load.
 */
export class OMEZarrDataset extends Dataset {
   ...
   * : format-specific details — pyramid diagnostics, omero channel
  ...
  override async load(): Promise<void> {
    ...
    if (config.type !== "ome-zarr") {
      throw new Error(`ome-zarr dataset cannot load config of type ${JSON.stringify(config.type)}`);
    }
    const url: unknown = config.source;
  ...
  /** Release references held for the resource (no stateful resources). */
...
/**
 * Open an OME-Zarr store AS a loaded {@link OMEZarrDataset} in one call
 * : the store's metadata is fetched once and retained on
...
): Promise<OMEZarrDataset> {
  const dataset = new OMEZarrDataset({ type: "ome-zarr", source: url }, options);
  await dataset.load();
  return dataset;
}
 ...
 * `registerDatasetAdapter` call below runs on import (the package's only declared
...
registerDatasetAdapter("ome-zarr", (config) => new OMEZarrDataset(config));
